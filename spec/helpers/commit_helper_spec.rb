...
describe GitlabEngine::CommitsHelper do
  ...
  it "should returns link_to issue for one valid issue in message" do
    ...
    expected = "One commit message <a href=\"/projects/#{@project.code}/issues/#{issue_id}\">##{issue_id}</a>"
  ...
  it "should handle multiple issue references in commit message" do
    ...
    expected = "One big commit message with a valid issue <a href=\"/projects/#{@project.code}/issues/#{issue_id}\">##{issue_id}</a>"+
        " and an invalid one ##{invalid_issue_id}.&#x000A;    "+
        "We reference valid <a href=\"/projects/#{@project.code}/issues/#{issue_id}\">##{issue_id}</a> multiple times "+
        "(<a href=\"/projects/#{@project.code}/issues/#{issue_id}\">##{issue_id}</a>) "+
