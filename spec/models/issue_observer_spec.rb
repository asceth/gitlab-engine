...
describe GitlabEngine::IssueObserver do
  ...
  subject { GitlabEngine::IssueObserver.instance }
  ...
  describe '#after_create' do
    ...
    it 'sends an email to the assignee' do
      GitlabEngine::Notify.should_receive(:new_issue_email).with(issue.id).
  ...
  describe '#send_reassigned_email' do
    ...
    def it_sends_a_reassigned_email_to(recipient)
      GitlabEngine::Notify.should_receive(:reassigned_issue_email).with(recipient, issue.id, previous_assignee.id).
        and_return(double(:deliver => true))
    end

    def it_does_not_send_a_reassigned_email_to(recipient)
      GitlabEngine::Notify.should_not_receive(:reassigned_issue_email).with(recipient, issue.id, previous_assignee.id)
    end
